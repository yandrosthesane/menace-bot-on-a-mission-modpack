#!/bin/bash
# BOAM Steam Launch Wrapper — start tactical engine, run game, cleanup on exit.
#
# Steam Launch Options:
#   /path/to/Mods/BOAM/boam-launch.sh %command%
#
# This script:
#   1. Starts the BOAM Tactical Engine in a terminal window
#   2. Launches the game via %command% (passed by Steam)
#   3. Shuts down the engine when the game exits

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ENGINE_DIR="$SCRIPT_DIR/tactical_engine"
ENGINE_BIN="$ENGINE_DIR/TacticalEngine"
PORT=7660
LOG_DIR="$SCRIPT_DIR/logs"
LOG_FILE="$LOG_DIR/tactical_engine.log"

# ─── Start tactical engine ───

# Stop any existing instance
if curl -s --max-time 1 "http://127.0.0.1:$PORT/status" > /dev/null 2>&1; then
    curl -s -X POST "http://127.0.0.1:$PORT/shutdown" > /dev/null 2>&1
    sleep 1
fi

if [ -f "$ENGINE_BIN" ]; then
    mkdir -p "$LOG_DIR"
    gnome-terminal --title="BOAM Tactical Engine" --geometry=120x30 -- \
        bash -c "\"$ENGINE_BIN\" --on-title /navigate/tactical 2>&1 | tee \"$LOG_FILE\"; echo -e '\n\x1b[31mTactical engine exited. Press Enter to close.\x1b[0m'; read"
    # Wait for engine to be ready
    for i in $(seq 1 10); do
        if curl -s --max-time 1 "http://127.0.0.1:$PORT/status" > /dev/null 2>&1; then
            break
        fi
        sleep 0.5
    done
fi

# ─── Launch game ───

"$@"
GAME_EXIT=$?

# ─── Shutdown engine ───

if curl -s --max-time 1 "http://127.0.0.1:$PORT/status" > /dev/null 2>&1; then
    curl -s -X POST "http://127.0.0.1:$PORT/shutdown" > /dev/null 2>&1
fi

exit $GAME_EXIT
