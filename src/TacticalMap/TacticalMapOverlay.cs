...
internal class TacticalMapOverlay
{
    ...
    /// <summary>
    /// Strip faction prefix, trailing instance number, common prefixes, and capitalise.
    /// "player.carda" → "Carda"
    /// "wildlife.alien_01_big_warrior_young.1" → "Big Warrior Young"
    /// "enemy_local.construct_scarecrow.2" → "Scarecrow"
    /// </summary>
    private static string ShortLabel(string uuid)
    {
    ...
    private static string TemplateFileName(string fullName)
    {
    ...
    private static string FactionIconName(int factionIdx) => factionIdx switch
    {
    ...
    private GUIStyle ResolveUnitIcon(OverlayUnit unit)
    {
        ...
        // 2. Try template icon
        var tplName = TemplateFileName(unit.Template);
        ...
        // 3. Faction fallback
        var facPath = Path.Combine(_iconsDir, "factions", FactionIconName(unit.FactionIndex) + ".png");
